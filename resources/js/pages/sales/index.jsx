    ...
    return (
        <>
            ...
            <div className="nexa-page">
                <PageHeader
                    title="Sales"
                    description="Performance insights based on issued invoices. Drafts are excluded."
                />
                <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                    <StatCard
                        label="Total sales"
                        value={money(summary.total_sales)}
                        detail="Issued invoice value"
                        icon={CircleDollarSign}
                        tone="success"
                    />
                    <StatCard
                        label="Issued invoices"
                        value={Number(summary.issued_invoices).toLocaleString()}
                        detail="Documents in this period"
                        icon={FileCheck2}
                    />
                    <StatCard
                        label="Units sold"
                        value={Number(summary.units_sold).toLocaleString()}
                ...
                <form
                    onSubmit={submit}
                    className="nexa-card grid gap-3 p-3 md:grid-cols-2 xl:grid-cols-[repeat(4,minmax(150px,1fr))_auto]"
                >
                    <DateInput
                        label="From"
                        value={data.date_from}
                        set={(value) => update('date_from', value)}
                    />
                    <DateInput
                        label="To"
                        value={data.date_to}
                        set={(value) => update('date_to', value)}
                    />
                    <Choice
                ...
                <div className="grid items-start gap-5 xl:grid-cols-2">
                    <DataPanel
                        title="Top customers"
                        description="Customers ranked by issued invoice value"
                        count={summary.by_customer.length}
                    >
                        <SimpleTable
                    ...
                    <DataPanel
                        ...
                        description="Items ranked by quantity sold"
                        count={summary.by_product.length}
                    >
                        <SimpleTable
                ...
                <DataPanel
                    ...
                    description="Latest sales contributing to this report"
                    count={summary.recent_invoices.length}
                >
                    <SimpleTable
                        headings={[
                            'Invoice',
                        ...
                        rows={summary.recent_invoices.map((item) => [
                            ...
                            <strong>{money(item.payable_amount)}</strong>,
                        ])}
                        empty="No issued sales found for the selected period."
                    />
                </DataPanel>
            </div>
        </>
...
function Choice({ value, set, all, options }) {
    return (
...
function SimpleTable({ headings, rows, empty }) {
    return (
