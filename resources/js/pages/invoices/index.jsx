...
export default function Index({ invoices, filters, summary }) {
    ...
    const destroy = (invoice) =>
        confirm(`Delete ${invoice.invoice_number}?`) &&
        router.delete(`/invoices/${invoice.id}`, { preserveScroll: true });
    const issue = (invoice) =>
        confirm(
            `Issue ${invoice.invoice_number}? Issued invoices cannot be edited.`,
        ) &&
        router.post(
    ...
    return (
        <>
            ...
            <div className="nexa-page">
                <PageHeader
                    title="Invoices"
                    description="Create, review and issue customer sales invoices."
                >
                    <Button asChild>
                ...
                <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
                    <StatCard
                        ...
                        detail="Every invoice record"
                        icon={Files}
                    />
                    <StatCard
                        label="Drafts"
                        value={summary.drafts.toLocaleString()}
                    ...
                    <StatCard
                        ...
                        detail="Pending and partial payments"
                        icon={CircleDollarSign}
                        tone={summary.outstanding > 0 ? 'warning' : 'default'}
                    />
                </div>

                <form onSubmit={submit} className="nexa-card space-y-3 p-3">
                ...
                <DataPanel
                    ...
                    description="Draft and issued sales documents"
                            ...
                            <tbody>
                                {!invoices.data.length && (
                                    <EmptyTable
                                        ...
                                        description={
                                            hasFilters
                                                ? 'Try changing or clearing the current invoice filters.'
                                                : 'Create a draft invoice to begin billing a customer.'
                                        }
                                        href={
                                            !hasFilters
                                                ? '/invoices/create'
...
function Td({ children, className = '' }) {
    return (
        <td
            className={`px-5 py-3.5 align-middle whitespace-nowrap ${className}`}
        >
            {children}
        </td>
    );
